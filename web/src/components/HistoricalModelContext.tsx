...
function target(label: string, wins: number, losses: number, pushes: number, count: number) {
  return (
    <div className="rounded-lg bg-surface-inset p-3">
      ...
      <div className="text-xs tabular-nums text-ink-muted">{wins}–{losses}–{pushes} · {count} compared</div>
    </div>
  );
}
      ...
      <div className="mt-3 grid gap-4 md:grid-cols-2">
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {target("Spreads", context.fullSeason.spreadWins, context.fullSeason.spreadLosses, context.fullSeason.spreadPushes, context.fullSeason.spreadComparedGames)}
            {target("Totals", context.fullSeason.totalWins, context.fullSeason.totalLosses, context.fullSeason.totalPushes, context.fullSeason.totalComparedGames)}
          </div>
        </div>
        <div>
          ...
          {period ? <div className="grid grid-cols-2 gap-3">
            {target("Spreads", period.spreadWins, period.spreadLosses, period.spreadPushes, period.spreadComparedGames)}
            {target("Totals", period.totalWins, period.totalLosses, period.totalPushes, period.totalComparedGames)}
