...
DEFAULT_CONFIG = (
    ROOT / "conf/research/data_first_football_v1/possession_rating_replay_2026_v1.yaml"
)
OUTPUT_ROOT = (
...
def _partitioned_ref(value: Mapping[str, Any], *, name: str) -> PartitionedDatasetRef:
    ...
    if missing := [field for field in fields if field not in value]:
        raise RatingReplayRunError(
            f"{name} partitioned reference lacks fields: {missing}"
        )
...
def _measurement_parent(storage: Any, uri: str) -> tuple[dict[str, Any], bytes]:
    ...
    if (
        ...
        raise RatingReplayRunError(
            "measurement parent is not the certified Contract 07 live parent"
        )
    return payload, raw


def _historical_parent(
    storage: Any, uri: str
    ...
    if (
        ...
        raise RatingReplayRunError(
            "historical parent is not the frozen 11B rating definition"
        )
    historical_measurement_uri = (rating.get("parents") or {}).get(
        "measurement_manifest_uri"
    )
    historical_measurement_sha = (rating.get("parents") or {}).get(
        "measurement_manifest_raw_sha256"
    )
    if not historical_measurement_uri or not historical_measurement_sha:
        raise RatingReplayRunError(
            "11B parent does not bind its historical measurement"
        )
    ...
    if (historical.get("identity") or {}).get(
        "run_id"
    ) != "possession-v1-measurements-20260921-r9":
        raise RatingReplayRunError(
            "11B historical measurement is not the certified r9 parent"
        )
...
def _preseason_refs(
    ...
    for name, uri in values.items():
        if not str(uri).startswith(_PRESEASON_PREFIXES[name]):
            raise RatingReplayRunError(
                f"{name} manifest does not bind a 2026 preseason capture"
            )
        raw = storage.read_bytes(uri)
        payload = json.loads(raw)
        if (
...
def _identity(
    ...
    *,
    ...
    historical_measurement_raw: bytes,
    preseason: Mapping[str, Any],
    ...
    value = {
        ...
        "historical_rating_manifest_raw_sha256": hashlib.sha256(
            historical_raw
        ).hexdigest(),
        "historical_measurement_manifest_raw_sha256": hashlib.sha256(
            historical_measurement_raw
        ).hexdigest(),
...
def _plans(computation: LiveReplayComputation) -> dict[str, DatasetPlan]:
    frames = {
        "priors": computation.priors,
        "rating_states": computation.rating_states,
        "team_states": computation.team_states,
    }
    ...
    for name, frame in frames.items():
        ...
        for partition_values, part in frame.groupby(
            ...
            ordered = part.sort_values(
                list(keys) + [column for column in schema.keys if column not in keys],
                kind="mergesort",
            )
            parts.append(
                {
        ...
        plans[name] = DatasetPlan(
            ...
            tuple(parts),
            int(len(frame)),
            partitioned_records_sha(parts, keys),
        )
    return plans
...
def _load_computation(
    ...
    historical_measurement: Mapping[str, Any],
) -> LiveReplayComputation:
    ...
    try:
        inputs = LiveReplayInputs(
            population=_read_partitioned(
                storage, outputs["population"], name="live population"
            ),
            observations=_read_partitioned(
                storage, outputs["observations"], name="live observations"
            ),
            snapshots=_read_partitioned(
                storage, outputs["snapshots"], name="live snapshots"
            ),
            historical_terminal=_read_partitioned(
                storage, historical_outputs["terminal"], name="r9 terminal"
            ),
        )
...
def preflight(*, storage: Any, args: argparse.Namespace) -> dict[str, Any]:
    ...
    measurement, measurement_raw = _measurement_parent(
        storage, args.measurement_manifest_uri
    )
    _, historical_raw, historical_measurement, historical_measurement_raw = (
        _historical_parent(storage, args.historical_rating_manifest_uri)
    )
    ...
    identity = _identity(
        ...
        historical_measurement_raw=historical_measurement_raw,
        preseason=preseason,
        config=config,
    )
...
def _evidence(path: Path, identity: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("selected_candidate") != FROZEN_CANDIDATE or set(
        payload.get("preflight_plans") or {}
    ) != set(OUTPUTS):
        raise RatingReplayRunError(
            "preflight evidence does not bind the replay outputs"
        )
    return payload
...
def _writers(
    ...
    *,
    ...
    parent_refs = (
        ...
        _dataset_ref(
            historical_measurement["output_refs"]["terminal"], name="r9 terminal"
        ),
    )
    return {
        name: PartitionedDatasetWriter(
            ...
            expected_parts={
                partition_key(part["partition"]): part for part in plans[name].parts
            },
        )
        for name in OUTPUTS
    }
...
def _existing(
    storage: Any, uri: str, identity: Mapping[str, Any]
) -> dict[str, Any] | None:
    if not storage.exists(uri):
        return None
    payload = json.loads(storage.read_bytes(uri))
    verify_signed_payload(payload, label="existing rating replay manifest")
    if (payload.get("identity") or {}).get("identity_sha256") != identity[
        "identity_sha256"
    ]:
        raise RatingReplayRunError(
            "rating replay run ID already has a different identity"
        )
    if payload.get("state") != "frozen" or set(payload.get("output_refs") or {}) != set(
        OUTPUTS
    ):
...
def apply(
    *, storage: Any, args: argparse.Namespace, evidence: Mapping[str, Any]
) -> dict[str, Any]:
    config = _load_config(Path(args.config))
    measurement, measurement_raw = _measurement_parent(
        storage, args.measurement_manifest_uri
    ...
    if storage.list_files(prefix):
        raise RatingReplayRunError(
            "rating replay prefix has a partial artifact and is permanently ineligible"
        )
    reviewed = _evidence(Path(args.preflight_evidence), identity)
    computation = _load_computation(storage, args, measurement, historical_measurement)
    plans = _plans(computation)
    if reviewed["row_counts"] != {
        name: plan.row_count for name, plan in plans.items()
    } or reviewed["output_records_sha256"] != {
        name: plan.records_sha for name, plan in plans.items()
    }:
        raise RatingReplayRunError(
            "apply recomputation differs from same-code preflight"
        )
    _immutable_json(
        storage,
        ...
        {
            "identity": identity,
            "plans": {name: plan.as_evidence() for name, plan in plans.items()},
            "production_activation_authorized": False,
        },
    )
    writers = _writers(
    ...
    for name, writer in writers.items():
        frame = frames[name]
        for values, part in frame.groupby(
            list(PARTITIONS[name]), sort=True, dropna=False
        ):
            ...
            writer.add(
                PartitionedDatasetPart(
                    partition,
                    part.sort_values(
                        list(PARTITIONS[name])
                        + [
                            column
                            for column in schema_for(
                                RATING_DATASETS[name][0], RATING_DATASETS[name][1]
                            ).keys
                            if column not in PARTITIONS[name]
                        ],
                        kind="mergesort",
                    ),
                )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args.apply:
        print(
            json.dumps(
                preflight(storage=storage, args=args),
                indent=2,
                sort_keys=True,
                default=str,
            )
    ...
    print(
        json.dumps(
            apply(storage=storage, args=args, evidence={}), indent=2, sort_keys=True
        )
    )
