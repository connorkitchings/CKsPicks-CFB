...
def test_best_available_prefers_line_then_price_then_bookmaker():
    quotes = pd.DataFrame(
        [
            {"market": "spreads", "side": "home", "point": -3.5, "price": -105, "bookmaker": "zeta"},
            {"market": "spreads", "side": "home", "point": -3.0, "price": -115, "bookmaker": "beta"},
            {"market": "spreads", "side": "home", "point": -3.0, "price": -110, "bookmaker": "alpha"},
        ]
    )
...
def test_total_over_prefers_lower_line_and_spread_settlement_uses_quote_side():
    quotes = pd.DataFrame(
        [
            {"market": "totals", "side": "over", "point": 49.5, "price": -110, "bookmaker": "a"},
            {"market": "totals", "side": "over", "point": 48.5, "price": -115, "bookmaker": "b"},
        ]
    )
...
def test_quote_selection_rejects_missing_executable_side():
    with pytest.raises(ValueError, match="No executable"):
        select_best_available_quote(
            pd.DataFrame(
                [{"market": "spreads", "side": "home", "point": -3, "price": -110, "bookmaker": "a"}]
            ),
            target="spread",
            direction="away",
        )
