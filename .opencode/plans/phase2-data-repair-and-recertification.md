# Phase 2: Data Repair and Recertification — Detailed Implementation Plan

## Status and Approval

- **Governing contract:** `docs/plans/2026-09-05/02-data-repair-and-recertification.md` (Approved)
- **Approval source:** User approved full Phase 2 scope on 2026-09-05:
  - Split into four ordered sub-phases (2a → 2b → 2c → 2d)
  - Ingest 2015-2019 corpus now
  - Include FBS-FCS games now (full FBS-involved population)
- **Input artifacts:** Phase 1 sealed audit at
  `artifacts/research/data-first-football-v1/phase1/2026-09-05T1510Z-phase1-evidence-audit-v2/`
- **Output namespace:** `artifacts/research/data-first-football-v1/phase2/`
...
1. **Investigate 52 catalog-missing lineage parents**
   ...
   - Classify each as:
     - **Stale edge:** child was rebuilt under new identity; old edge is orphaned.
       Prune via `DELETE FROM catalog.dataset_dependencies WHERE ...` (append-only
       schema permits deletion of stale edges only).
     - **Missing registration:** parent version was never registered in the new
       catalog system. Register it from the existing R2 object (read manifest,
       call `register_dataset_version`).
   - Write a research script `scripts/research/repair_phase2_catalog.py` with
     `--dry-run` and `--apply` modes.
   - Add regression tests for the repair logic.
...
- All 52 lineage edges classified and either pruned or registered.
...
- Re-run Phase 1 audit v3 (new run-id) against the repaired catalog; verify
  issue count drops by at least 53 (52 lineage + 1 dataset-correctness).
...
- [ ] 52 lineage edges classified and resolved.
...
- [ ] Session log `session_logs/YYYY-MM-DD/NN.md` written.
...
1. **Add `fbs_involved_games` Silver contract**
   ...
   - Required columns: same as `games_v2` plus `population` (fbs_fbs / fbs_fcs / unresolved)
...
4. **Publish eligibility manifest**
   - Write `artifacts/research/data-first-football-v1/phase2/eligibility-manifest.json`:
...
5. **Re-run Phase 1 audit v4**
   ...
   - Verify:
     - All 57 issues resolved or explicitly quarantined.
     - All 5 research results now `reproducible` or `requires-correction` (not `unsupported`).
...
## Amendment to Governing Contract

This detailed plan is an amendment to
`docs/plans/2026-09-05/02-data-repair-and-recertification.md`. The amendment
adds the four sub-phases with explicit scope, tasks, acceptance, risks, and
definition of done. The governing contract's original six implementation tasks
are satisfied by the sub-phases as follows:
...
Before implementing Phase 2, repair two small documentation drifts from Phase 1
closure (fast path, 5 minutes):

1. Update `docs/plans/index.md` lines 37-40:
   - Phase 1: "Approved; begins after Phase 0" → "Implemented 2026-09-05. Sealed audit at [run-id]; 57 issues feed Phase 2."
   - Phase 2: "Approved; consumes verified Phase 1 defects" → "Approved; consumes Phase 1 findings from [sealed run]."

2. Update `AGENTS.md` "2026 Season Execution Status" to mention Phase 1 completion
   and the sealed audit artifact.

One small commit.

## Handoff

After this plan is approved and the user commits it, open a fresh Terra task
to implement Sub-phase 2a (deterministic repairs).
