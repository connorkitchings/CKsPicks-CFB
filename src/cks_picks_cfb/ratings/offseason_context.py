...
FAMILY_FEATURES: Mapping[str, tuple[str, ...]] = {
    "returning_production": ("return_total_ppa", "return_passing_ppa", "return_rushing_ppa", "return_receiving_ppa", "return_percent_ppa", "return_passing_usage", "return_rushing_usage"),
    "transfer_portal": ("transfer_in_count", "transfer_out_count", "transfer_net_rating", "transfer_in_qb", "transfer_out_qb"),
...
def _source(family: str, frame: pd.DataFrame, seasons: set[int]) -> pd.DataFrame:
    ...
    required = {"season", "team", "effective_at", "retrieved_at", *FAMILY_FEATURES[family]}
    if missing := sorted(required - set(frame)):
        raise ContextAdmissionError(f"{family} is missing columns: {missing}")
    if forbidden := [c for c in frame if any(t in c.casefold() for t in FORBIDDEN_TOKENS)]:
        raise ContextAdmissionError(f"{family} contains market-derived columns: {forbidden}")
    result = frame.loc[:, ["season", "team", "effective_at", "retrieved_at", *FAMILY_FEATURES[family]]].copy()
    result["season"] = pd.to_numeric(result["season"], errors="coerce")
    if result["season"].isna().any() or not set(result["season"].astype(int)) <= seasons:
    ...
    if result["team"].isna().any() or result.duplicated(["season", "team"]).any():
        raise ContextAdmissionError(f"{family} has invalid or duplicate team-season keys")
...
def admit_offseason_context(
    ...
    *,
    ...
    if 2020 in permitted or authentic_season in permitted or not 0 < minimum_coverage <= 1:
    ...
    if universe["season"].isna().any() or universe["team"].isna().any() or universe.duplicated(["season", "team"]).any():
        raise ContextAdmissionError("Team universe has invalid or duplicate keys")
    universe["season"] = universe["season"].astype(int)
    universe["first_kickoff_utc"] = _timestamps(universe["first_kickoff_utc"], "first_kickoff_utc")
    ...
    for family in FAMILY_FEATURES:
        ...
        try:
            ...
            continue
        joined = universe.merge(source, on=["season", "team"], how="left", validate="one_to_one")
        ...
        for season, values in joined.groupby("season", observed=True):
            ...
            season_report[str(int(season))] = {"required_rows": len(values), "covered_rows": int(covered.sum()), "coverage_fraction": coverage, "pre_kickoff_evidence_rows": int(evidence.sum())}
            complete &= coverage >= minimum_coverage
            strict &= bool(evidence.all())
        if not complete:
            reports[family] = {"status": "rejected", "reason": f"coverage below {minimum_coverage:.0%}", "seasons": season_report}
            continue
        status = "strict" if strict else "reconstructed"
        reports[family] = {"status": status, "reason": None if strict else "pre-kickoff source evidence incomplete", "required_features": features, "seasons": season_report, "source_ref": dict((source_refs or {}).get(family, {}))}
    ...
    for frame in admitted[1:]:
        context = context.merge(frame, on=["season", "team"], how="outer", validate="one_to_one")
    families = [name for name, detail in reports.items() if detail["status"] != "rejected"]
    track = "strict" if all(reports[name]["status"] == "strict" for name in families) else "reconstructed"
    context = context.copy()
    context["feature_track"] = track
    return ContextAdmission(
        context=context.sort_values(["season", "team"]).reset_index(drop=True),
        report={"schema_version": REPORT_VERSION, "state": "admitted", "feature_track": track, "activation_eligible": track == "strict", "minimum_coverage": minimum_coverage, "permitted_seasons": sorted(permitted), "authentic_season": authentic_season, "admitted_families": families, "families": reports},
    )


def require_admitted_context(report: Mapping[str, object], *, allow_reconstructed: bool) -> tuple[str, ...]:
    """Reject unadmitted or reconstructed context outside explicit research mode."""
    if report.get("schema_version") != REPORT_VERSION or report.get("state") != "admitted":
    ...
    if track == "reconstructed" and not allow_reconstructed:
        raise ContextAdmissionError("Reconstructed context requires explicit research-only opt-in")
