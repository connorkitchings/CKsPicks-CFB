...
def _historical_scale(terminal: pd.DataFrame, *, role: str) -> tuple[float, float, float]:
    rows = terminal[
...
def _live_games(population: pd.DataFrame) -> pd.DataFrame:
    ...
    games = population[pd.to_numeric(population["season"], errors="coerce").eq(LIVE_SEASON)].copy()
    if games.empty:
    ...
    if not (eligible["schedule_completed"].eq(True) & eligible["outcome_valid"].eq(True)).all():
    ...
    return eligible.sort_values(["kickoff_utc", "game_id"], kind="mergesort").reset_index(drop=True)
...
def _boundaries(games: pd.DataFrame) -> pd.DataFrame:
    ...
    for team in teams:
        ...
        for game in schedule.itertuples(index=False):
            ready_at = pd.Timestamp(game.kickoff_utc) + pd.Timedelta(hours=_AVAILABILITY_HOURS)
            allowed = schedule[
...
def _priors(games: pd.DataFrame, terminal: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, tuple[float, float, float]]]:
    required = {
        "season",
        "team",
        "measurement_id",
        ...
        for team in teams:
            if team in previous.index:
                ...
                if not np.isfinite(value) or not np.isfinite(exposure) or exposure <= 0:
                    carry = RatingPrior(0.0, 1.0, "neutral", None, "invalid_predecessor")
                else:
            ...
            prior_lookup[f"{role}\u0000{team}"] = (float(carry.mean), float(carry.variance), sign)
    frame = pd.DataFrame.from_records(rows, columns=list(PRIOR_COLUMNS)).sort_values(
        ["unit_role", "team"], kind="mergesort"
    ).reset_index(drop=True)
    return frame, prior_lookup


def _streams(
    *, observations: pd.DataFrame, snapshots: pd.DataFrame, boundaries: pd.DataFrame, scale: Mapping[str, tuple[float, float, float]]
) -> dict[str, dict[str, list[dict[str, Any]]]]:
    required_observations = {
        "season", "game_id", "kickoff_utc", "team", "measurement_id", "unit_role", "denominator", "coverage_status", "timing_class"
    }
    required_snapshots = {
        "season", "as_of_game_id", "team", "measurement_id", "unit_role", "adjustment_iteration", "adjusted_value", "timing_class"
    }
    ...
    if not observations["timing_class"].eq("live").all() or not snapshots["timing_class"].eq("live").all():
        raise LiveReplayError("live measurement inputs have a non-live timing class")
    snapshot_values = {
        (int(row.as_of_game_id), str(row.team), str(row.measurement_id), str(row.unit_role)): float(row.adjusted_value)
        for row in snapshots.itertuples(index=False)
        if int(row.season) == LIVE_SEASON and int(row.adjustment_iteration) == 4 and pd.notna(row.adjusted_value)
    }
    boundary_by_game = (
        boundaries.set_index(["game_id", "team"]) if not boundaries.empty else pd.DataFrame()
    )
    subset = observations[
    ...
    for row in subset.itertuples(index=False):
        ...
        value = snapshot_values.get((int(boundary.boundary_game_id), str(row.team), FROZEN_DEFINITION, str(row.unit_role)))
    ...
    for role in ROLES:
        for team in result[role]:
            result[role][team].sort(key=lambda item: (item["kickoff_utc"], item["game_id"]))
    return result
...
def build_live_replay(inputs: LiveReplayInputs) -> LiveReplayComputation:
    ...
    scale = {role: _historical_scale(inputs.historical_terminal, role=role) for role in ROLES}
    streams = _streams(
        ...
        for team in (str(game.home_team), str(game.away_team)):
            ...
            for role in ROLES:
                ...
                observed = [
                    ...
                    if item["boundary_cutoff_utc"] <= cutoff and item["game_id"] != int(game.game_id)
                ]
                exposure = float(sum(item["exposure"] for item in observed))
                weighted = float(sum(item["adjusted_z"] * item["exposure"] for item in observed))
    ...
    states = pd.DataFrame.from_records(state_rows, columns=list(RATING_STATE_COLUMNS)).sort_values(
        ["week", "game_id", "team", "unit_role"], kind="mergesort"
    ).reset_index(drop=True)
    teams = pd.DataFrame.from_records(team_rows, columns=list(TEAM_STATE_COLUMNS)).sort_values(
        ["week", "game_id", "team"], kind="mergesort"
    ).reset_index(drop=True)
    return LiveReplayComputation(
        ...
        diagnostics={
            ...
            "source_observations": int(sum(len(items) for role in streams.values() for items in role.values())),
        },
    )
