...
def evaluate_gate(findings: list[Mapping[str, Any]]) -> dict[str, Any]:
    """Recompute the Contract 11 entry-gate evaluation from findings.

    Contract 11 may begin only when no upstream Repair/measurement/rating
    blocker remains open, forecast findings are resolved or incorporated, and
    the audit manifest agrees with this record exactly.
    """
    upstream_stages = ("repair", "measurements", "ratings")
    upstream_blocker_open = any(
        finding.get("severity") == "blocker"
        and finding.get("disposition") != "eligible_for_next_contract"
        and any(
...
def verify_published_audit(
    ...
    *,
    ...
    """Verify a published Preview audit prefix: manifest-last publication.

    Confirms the terminal manifest digest, identity, output bytes/hashes,
    output counts, finding dispositions, and gate arithmetic, plus
    source-reference existence when storage is provided.
    """
    if manifest.get("schema_version") != AUDIT_MANIFEST_SCHEMA:
        raise AuditVerificationError("unexpected audit manifest schema")
    declared_manifest_sha = manifest.get("manifest_sha256")
