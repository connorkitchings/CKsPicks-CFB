...
These legacy files were generated by `scripts/pipeline/run_pipeline_generic.py`, which is archived. Current durable datasets are built through the Silver/Gold workflow documented in `docs/ops/weekly_pipeline.md`.
-   `byplay`
-   `drives` (aggregated)
-   `team_game`
-   `team_season`
...
3.  **Validation**:
    -   Historical validation only: use the current Silver/Gold builder and point-in-time regression tests instead of the archived generic wrapper.
