...
5. **Best-quote line policy (`model_side_best_quote_v1`) — unified 1.0 no-bet rule rehearsed, release pending (2026-09-26).** Edges below 1.0 point now publish no lean for either target (artifact "No Bet" labels authoritative; quote lineage kept); totals in [1.0, 1.5) display a side without a grade; the total grade threshold stays 1.5. The Preview rebuild batch (`2026w{0..4}-v5replay-bestquote-20260926-r3`) is verified: forecasts identical to the `-r2` batch, 11 sub-1.0 spread grades removed (146 spread / 124 total), all lines real ticks, serving + rollback drill proven. Production packets validate; the re-release awaits a fresh authorization decision. The live site still serves the `-r2` batch. See the [no-bet contract](../plans/2026-09-26/02-unified-no-bet-threshold.md).
