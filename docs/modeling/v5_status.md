...
5. **Best-quote line policy (`model_side_best_quote_v1`) — Preview rehearsed (2026-09-26).** The `prediction_market_selections` schema (migration 0016) and the `select_best_quote()` selection service are implemented, tested, and deployed to both environments. Future runs select the best pre-kickoff executable quote for each target before freezing. The full Preview replacement rehearsal is complete (Amendment 3 of the [selection contract](../plans/2026-09-25/03-permanent-best-quote-line-selection.md)): Weeks 0–4 regenerated from the pinned verified replays with forecasts byte-identical to the selected runs, every line a real market tick, grades settled at the exact selected quote, serving selection and rollback drill proven. Production replacement runs, exact packets, and per-week activation remain gated on separate authorization; W4 replacement scoring waits at its own finals gate.
